...
class PitchAccentApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Pitch Accent Trainer (Segment Loop + Playback)")
    ...
    def setup_gui(self):
        self.root.geometry("1600x1000")  # double the original size
...
self.play_user_button = tk.Button(control_frame, text="Play Your Recording", command=self.play_user_audio)
        ...
        self.span_selector = SpanSelector(self.ax_native, self.on_select_span, "horizontal", useblit=True,
    ...
    def load_native(self):
        self.playing = False  # stop playback if active
        sd.stop()
    ...
    def update_native_plot(self):
        ...
        self.ax_native.set_xlim(0, duration)
    ...
    def loop_native_audio(self):
        ...
        while self.playing:
            ...
            end_sample = int(self.loop_end * sr) if self.loop_end else len(y)
            y_segment = y[start_sample:end_sample]
            sd.play(y_segment, sr, device=output_index)
            try:
                while not sd.get_stream().active:
                    time.sleep(0.005)
            except RuntimeError:
                time.sleep(0.05)  # fallback if stream not accessible
...
def on_select_span(self, xmin, xmax):
        self.loop_start = max(0, xmin)
        self.loop_end = max(self.loop_start, xmax)
