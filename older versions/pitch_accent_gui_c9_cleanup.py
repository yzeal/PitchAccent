...
class PitchAccentApp:
    ...
    def record_audio(self):
        ...
        self.recording = True
        self.ax_user.clear()
        self.ax_user.grid(True)  # Make sure grid is enabled from the start
        self.ax_user.set_ylim(50, 500)  # Set initial y-axis limits
        self.ax_user.set_xlim(0, 5)  # Set initial x-axis window
        self.canvas.draw()
        ...
        try:
            ...
            def update_realtime_pitch():
                print("Attempting to update pitch visualization...")  # Debug print
                if len(frames) > int(fs * 0.1):  # Reduce chunk size to 0.1s for more frequent updates
                    try:
                        # Get the most recent chunk of audio
                        recent_audio = np.concatenate(frames[-int(fs * 0.1):])
                        temp_file = os.path.join(tempfile.gettempdir(), "temp_chunk.wav")
                        wavfile.write(temp_file, fs, recent_audio)
                        
                        # Extract pitch
                        x, y = self.extract_smoothed_pitch(temp_file)
                        current_time = len(frames) * len(recent_audio) / fs
                        print(f"Current recording time: {current_time:.2f}s")  # Debug print
                        ...
                        # Plot all accumulated pitch data
                        pitch_buffer.extend(list(zip(x + current_time - len(x) * 0.01, y)))
                        if pitch_buffer:
                            x_plot, y_plot = zip(*pitch_buffer)
                            self.ax_user.plot(x_plot, y_plot, color='orange', linewidth=2)
                            self.ax_user.set_xlim(max(0, current_time - 5), current_time + 1)
                            print(f"Plotting pitch data: {len(pitch_buffer)} points")  # Debug print
            ...
            def callback(indata, frames_, time_, status):
                ...
                if len(frames) % int(fs * 0.1) == 0:  # Update every 0.1 seconds
                    self.root.after(1, update_realtime_pitch)  # Schedule update on main thread
            ...
            def start_recording():
                nonlocal frames, pitch_buffer
                frames = []  # Reset frames
                pitch_buffer = []  # Reset pitch buffer
                ...
                try:
                    with sd.InputStream(samplerate=fs, channels=1, dtype='float32',
                        ...
                        while time.time() - start_time < duration:
                            sd.sleep(100)  # Sleep in smaller increments
                            
                    print("Recording finished, processing final audio...")
                    if frames:
